...
class TaskEmbeddingVisualizer:
    def __init__(self, curriculum_file_path):
        ...
        self.embeddings = np.array(
            [single_spec.embedding for single_spec in self.curriculum]
        )

    def visualize(self, dims=2):
        task_names = [single_spec.name for single_spec in self.curriculum]
        colors = [
            f"#00{format(int(val), '02x')}ff"
            for val in np.linspace(0, 255, len(task_names))
        ]
        ...
        trace = go.Scatter3d(
            ...
            z=z,
            mode="markers",
            marker=dict(color=colors, size=6, symbol="circle", opacity=0.8),
